"""
Thumbnail generation using WaveSpeed AI (Flux 2 Klein 9B)
...
from image_gen import generate_image, WIDTH, HEIGHT

THUMBNAIL_STYLE = (
    "YouTube thumbnail style, high contrast, eye-catching, dramatic composition. "
    "Bold thick black outline stickman illustration. "
    "Use one or two vivid flat accent colors against a bright bold background color "
    "(not white — use a punchy color like deep blue, red, or orange). "
    "Flat 2D design, no text in the image, simple but dramatic. Scene: "
)


def generate_thumbnail_image(visual_prompt: str, output_path: str, seed: int = 99) -> bool:
    full_prompt = THUMBNAIL_STYLE + visual_prompt
    # Reuse the same WaveSpeed call but with thumbnail-specific styling
    # (bypass image_gen's STYLE_PREFIX by calling the low-level pieces directly)
    from image_gen import _submit_job, _poll_result, _normalize_image_aspect
    import time
    import requests
...
def add_text_overlay(image_path: str, text: str, output_path: str):
    ...
    cmd = ["ffmpeg", "-y", "-i", image_path, "-vf", vf, "-frames:v", "1", output_path]
