...
MAX_SEGMENTS = 15  # hard ceiling regardless of runtime target - some segments
                    # also get dropped later for missing images, so this isn't
                    # a guaranteed final count, just the most we'll ever request
...
def run():
    ...
    if len(segments) >= MAX_SEGMENTS and estimated < TARGET_MIN_SECONDS:
        print(f"  Hit the {MAX_SEGMENTS}-segment cap before reaching the runtime target - proceeding anyway.")
